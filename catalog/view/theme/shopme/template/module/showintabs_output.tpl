<?php if (!empty($title)) { ?>
<h3 class="panel_header"><?php echo $title; ?></h3>
<?php } ?>
...
<select id="tabs-<?php echo $module; ?>" data-tabs="tabs">
    <?php foreach ($tabs as $keyTab => $tab) { ?>
        <option value="<?php echo $keyTab ?>">
            <a href="#tab<?php echo $module; ?><?php echo $keyTab; ?>" data-toggle="tab"><?php echo $tab['title']; ?></a>
        </option>
    <?php } ?>
</select>
...
<script type="text/javascript">
    $('#tabs-<?php echo $module; ?>').on('change', function (evt) {
      if ($('#tabs-<?php echo $module; ?>').select2('val') != null){
        <?php foreach ($tabs as $keyTab => $tab) { ?>
            var id = '#tab<?php echo $module; ?>' + '<?php echo $keyTab; ?>';
            var element = $(id);
            if(element.hasClass('active') && element.hasClass('in')){
                $(id).removeClass('active');
                $(id).removeClass('in');
            }
        <?php } ?>

        var id = '#tab<?php echo $module; ?>' + $('#tabs-<?php echo $module; ?>').select2('val');
        $(id).addClass('active in');
      }
    });

    $('#tabs-<?php echo $module; ?>').select2({
        minimumResultsForSearch: Infinity,
        placeholder: '<?php echo $tabs[0]['title']; ?>'
    });
